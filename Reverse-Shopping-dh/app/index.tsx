...
const fetchAllRecipes = async (): Promise<Recipe[]> => {
  try {
    ...
    console.log('Fetching all recipes from:', apiUrl);
    ...
    console.log(`API returned ${apiRecipes.length} recipes`);
    
    // Also update local storage with latest recipes
    try {
      const mappedRecipes = mapApiRecipesToLocal(apiRecipes);
      await AsyncStorage.setItem('userRecipes', JSON.stringify(mappedRecipes));
      console.log('Updated local recipe cache with API results');
      return mappedRecipes;
    } catch (storageError) {
      console.error('Error updating local recipe cache:', storageError);
      return mapApiRecipesToLocal(apiRecipes);
    }
  } catch (error) {
    ...
    try {
      ...
      if (storedRecipes) {
        ...
        console.log(`Loaded ${parsedRecipes.length} recipes from local storage as fallback`);
        return parsedRecipes;
      }
    } catch (storageError) {
      console.error('Error loading recipes from storage:', storageError);
    }
...
export default function App() {
  ...
  // Load recipes on startup to show users the latest content
  useEffect(() => {
    const loadInitialRecipes = async () => {
      if (!isLoading && recipes.length === 0) {
        ...
        try {
          ...
          // Mark as having searched so recipe updates will trigger refresh
          setHasSearched(true);
        } catch (error) {
    ...
    // Only load recipes after fonts are loaded and storage check is complete
    if (fontsLoaded && !isLoadingStorage) {
      loadInitialRecipes();
    }
  }, [fontsLoaded, isLoadingStorage]);
  ...
  // Function to check if recipes have been updated (added/modified)
  const checkForRecipeUpdates = async () => {
    try {
      // Check if we need to force refresh recipes (from profile page)
      const forceRefresh = await AsyncStorage.getItem('force_refresh_recipes');
      console.log('Checking for force refresh flag:', forceRefresh);
      
      if (forceRefresh === 'true') {
        console.log('Force refresh recipes flag detected - reloading recipes');
        ...
        try {
          ...
          const allRecipes = await fetchAllRecipes();
          console.log(`Fetched ${allRecipes.length} recipes from API`);
          setRecipes(allRecipes);
          
          // Update filtered recipes if we're in search mode
          if (hasSearched) {
            handleSearch();
          }
        } catch (fetchError) {
          console.error('Error fetching recipes during force refresh:', fetchError);
      ...
      // Get the last recipes update timestamp from state
      const currentRecipesTimestamp = await AsyncStorage.getItem('recipes_last_checked_timestamp') || '';
      
      // Get the latest timestamp from AsyncStorage
      const latestRecipesTimestamp = await AsyncStorage.getItem('recipes_last_updated') || '';
      
      // If no timestamp exists or it's the same as our last check, no need to update
      if (!latestRecipesTimestamp || latestRecipesTimestamp === currentRecipesTimestamp) return;
      
      console.log('Recipes timestamp changed, reloading all recipes');
      
      // Update our last check timestamp
      await AsyncStorage.setItem('recipes_last_checked_timestamp', latestRecipesTimestamp);
      
      // Only reload if we've already searched or loaded recipes
      if (hasSearched || recipes.length > 0) {
        // If we were searching by ingredients, repeat the search
        if (searchText.trim()) {
          handleSearch();
        } else {
          // Otherwise reload all recipes
          setIsLoading(true);
          const allRecipes = await fetchAllRecipes();
          setRecipes(allRecipes);
          setIsLoading(false);
      ...
      console.error('Error checking for recipe updates:', error);
    }
  };
  
  // Check for updates when the app regains focus or when component mounts
  useEffect(() => {
    // Initial check for recipe updates when component mounts
    const initialCheck = async () => {
      console.log('Initial check for recipe updates');
    ...
    const intervalId = setInterval(() => {
      ...
      if (!selectedRecipe) {
        ...
        checkForRecipeUpdates(); // Also check for recipe updates
      }
    }, 2000); // Check every 2 seconds
    
    // Clean up the interval when component unmounts
    return () => clearInterval(intervalId);
  }, [selectedRecipe, lastSyncTimestamp]);  // Removed dependencies that cause frequent re-renders
