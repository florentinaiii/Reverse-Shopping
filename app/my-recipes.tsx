...
export default function MyRecipesScreen() {
    ...
    // Translation state variables have been removed
    ...
    const loadSavedRecipes = useCallback(async () => {
        ...
        try {
            // Merr ID-të e recetave të ruajtura
            const storedValue = await AsyncStorage.getItem(SAVED_RECIPES_KEY);
            const savedRecipeIds: string[] = storedValue ? JSON.parse(storedValue) : [];
            
            // Ngarko recetat e plota nga API
            const recipePromises = savedRecipeIds.map(id => fetchRecipe(id));
            const recipes = await Promise.all(recipePromises);
            
            // Filtro recetat null (që nuk u gjetën)
    ...
    const handleUnsave = async (recipe: Recipe) => {
        try {
            const storedValue = await AsyncStorage.getItem(SAVED_RECIPES_KEY);
            let savedRecipeIds: string[] = storedValue ? JSON.parse(storedValue) : [];

            // Hiq ID-në e recetës nga lista e ID-ve të ruajtura
            const updatedRecipeIds = savedRecipeIds.filter(id => id !== recipe.id);
            
            // Ruaj ID-të e përditësuara në AsyncStorage
            await AsyncStorage.setItem(SAVED_RECIPES_KEY, JSON.stringify(updatedRecipeIds));
            
            // Përditëso listën e recetave të shfaqura
    ...
    return (
        ...
        >
            ...
            {viewingRecipe && typeof viewingRecipe === 'object' ? (
                    ...
                    <View style={styles.detailContentWrapper}>
                        <View style={styles.detailTopRowContainer}>
                            <View style={styles.detailImageContainer}>
                                <Image
                                    source={{ uri: viewingRecipe.image }}
                                    style={styles.detailImage}
                                    resizeMode="cover"
                                    defaultSource={require('../assets/images/recipedd.jpg')}
                                />
                            </View>
                            <View style={styles.detailIngredientsContainer}>
                                <View style={styles.section}>
                                    ...
                                    {viewingRecipe.ingredients && viewingRecipe.ingredients.length > 0 ? (
                                        ...
                                        <Text style={styles.ingredientText}>Nuk ka përbërës të disponueshëm</Text>
                                    )}
                                </View>
                            </View>
                        </View>
                        ...
                        <View style={styles.detailInstructionsContainer}>
                            <View style={styles.section}>
                                ...
                                <Text style={styles.instructionsText}>
                                    {viewingRecipe.instructions || 'Nuk ka udhëzime të disponueshëm'}
                                </Text>
                            </View>
                        </View>
                    </View>
