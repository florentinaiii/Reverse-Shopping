...
const ProfileScreen = () => {
  ...
  const handlePublishRecipe = async () => {
    try {
      ...
      ]) as (PromiseFulfilledResult<any> | PromiseRejectedResult)[];

      const imageUrl = results[0];
      const apiResponse = results[1];

      // Use type guards before accessing status/data
      if (apiResponse && apiResponse.status === 'fulfilled') {
        ...
        console.error('API Request rejected:', (apiResponse as PromiseRejectedResult).reason);
        if ((apiResponse as any)?.reason?.isOfflineError) {
      ...
      if ((apiResponse && apiResponse.status === 'fulfilled' &&
        ...
        const uploadedImageUrl = imageUrl && imageUrl.status === 'fulfilled' ? imageUrl.value : null;

        let serverRecipe: any;

        if (apiResponse && apiResponse.status === 'fulfilled' && apiResponse.value?.data) {
          serverRecipe = {
        ...
        if (imageUrl && imageUrl.status === 'fulfilled' && imageUrl.value) {
