...
// API base URL - using config file
const getApiBaseUrl = () => {
  if (__DEV__) {
...
const kerkoReceta = async (ingredients: string): Promise<Recipe[]> => {
  try {
    ...
    if (!ingredients || ingredients.trim() === '') {
      ...
      console.log(`Fetched ${data ? data.length : 0} recipes from MongoDB`);
      
      if (data && data.length > 0) {
        console.log('Sample recipe from API:', data[0]);
      } else {
        console.log('No recipes returned from API');
      }
      
      return mapApiRecipesToLocal(data || []);
    }
